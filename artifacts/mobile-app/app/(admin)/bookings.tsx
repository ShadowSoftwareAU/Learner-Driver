...
function AdminBookingCard({ booking, colors }: { booking: any; colors: ReturnType<typeof useColors> }) {
  ...
  return (
    <View style={[card.container, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={card.top}>
        <View style={{ flex: 1 }}>
          <Text style={[card.names, { color: colors.foreground }]} numberOfLines={1}>
            {booking.studentName ?? "Student"} → {booking.instructorName ?? "Instructor"}
          </Text>
          <Text style={[card.dt, { color: colors.mutedForeground }]}>
...
const card = StyleSheet.create({
  ...
  names: { fontSize: 14, fontFamily: "Inter_500Medium" },
  dt: { fontSize: 12, fontFamily: "Inter_400Regular", marginTop: 3 },
    ...
    <View style={[s.container, { backgroundColor: colors.background }]}>
      <View style={[s.header, { paddingTop: topPad + 8 }]}>
        <View style={s.titleRow}>
          ...
          {bookings ? <Text style={[s.count, { color: colors.mutedForeground }]}>{(bookings as any[]).length} total</Text> : null}
        </View>
        <View style={s.filterRow}>
          {FILTERS.map((f) => (
            <Pressable
              key={f.key}
              style={[s.filterBtn, { borderColor: colors.border }, filter === f.key && { backgroundColor: colors.primary, borderColor: colors.primary }]}
              onPress={() => setFilter(f.key)}
            >
              <Text style={[s.filterText, { color: filter === f.key ? "#FFF" : colors.foreground }]}>{f.label}</Text>
            </Pressable>
          ))}
        </View>
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <AdminBookingCard booking={item} colors={colors} />}
