...
function Avatar({ name, size = 64 }: { name: string; size?: number }) {
  ...
  return (
    ...
    >
      <Text style={{ fontSize: size * 0.35, fontFamily: "Inter_700Bold", color: "#2563EB" }}>
        {initials}
      </Text>
    </View>
  );
...
export default function InstructorProfileScreen() {
  ...
  const displayName = user ? user.name || user.email || "Instructor" : "Instructor";
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={[styles.badge, { backgroundColor: colors.accent }]}>
                <Text style={[styles.badgeText, { color: colors.accentForeground }]}>{roleLabel}</Text>
              </View>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  heading: { fontSize: 28, fontFamily: "Inter_700Bold", marginBottom: 24, marginTop: 8 },
