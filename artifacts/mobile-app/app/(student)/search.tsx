...
function InstructorCard({ instructor, colors }: { instructor: any; colors: ReturnType<typeof useColors> }) {
      ...
      <View style={{ flex: 1 }}>
        ...
        {instructor.suburb ? (
          <View style={card.row}>
            ...
            <Text style={[card.sub, { color: colors.mutedForeground }]}>{instructor.suburb}</Text>
          </View>
        ) : null}
        {rate ? (
          <View style={card.row}>
            <Feather name="dollar-sign" size={12} color={colors.mutedForeground} />
            <Text style={[card.sub, { color: colors.mutedForeground }]}>{rate}</Text>
          </View>
        ) : null}
      </View>
  ...
  return (
    <View style={[s.container, { backgroundColor: colors.background }]}>
      <View style={[s.header, { paddingTop: topPad + 8 }]}>
        <Text style={[s.title, { color: colors.foreground }]}>Find Instructor</Text>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <InstructorCard instructor={item} colors={colors} />}
