...
function StudentBookingCard({ booking, colors }: { booking: any; colors: ReturnType<typeof useColors> }) {
  ...
  return (
    <View style={[card.container, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={card.top}>
        <View style={[card.iconBox, { backgroundColor: colors.primary + "15" }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[card.duration, { color: colors.mutedForeground }]}>
            {booking.durationMinutes ?? 60} min lesson
          </Text>
        </View>
      ...
      {booking.vehicleSummary ? (
        <View style={[card.vehicle, { borderTopColor: colors.border }]}>
          <Feather name="truck" size={13} color={colors.mutedForeground} />
          <Text style={[card.vehicleText, { color: colors.mutedForeground }]}>
            {booking.vehicleSummary}
          </Text>
        </View>
      ) : null}
...
const card = StyleSheet.create({
  container: { borderRadius: 12, borderWidth: 1, padding: 14, gap: 8 },
  ...
  vehicle: { flexDirection: "row", alignItems: "center", gap: 6, borderTopWidth: 1, paddingTop: 8 },
  ...
  return (
    <View style={[s.container, { backgroundColor: colors.background }]}>
      <View style={[s.header, { paddingTop: topPad + 8 }]}>
        <Text style={[s.title, { color: colors.foreground }]}>My Lessons</Text>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <StudentBookingCard booking={item} colors={colors} />}
          ItemSeparatorComponent={() => <View style={{ height: 8 }} />}
          ListEmptyComponent={
            <View style={s.centered}>
              <Feather name="calendar" size={40} color={colors.border} />
              <Text style={[s.emptyText, { color: colors.mutedForeground }]}>
                {filter === "upcoming" ? "No upcoming lessons" : "No lessons found"}
              </Text>
            </View>
          }
        />
